...
const createColumn = async (req, res, next) => {
    try{
        ...
        const result = await pool.query(`INSERT INTO columns(name, board_id) VALUES($1, $2) RETURNING *;`,[name,board_id]);
...
const getAllColumns = async (req, res, next) => {
    ...
    try{ 
      ...
      const result = await pool.query(`SELECT * FROM columns LIMIT $1 OFFSET $2;`, [limit, offset]);
...
const UpdateColumns = async (req, res, next) => {
    ...
    const ColumnCheck = await pool.query(`SELECT * FROM columns WHERE id=$1;`, [id]);
...
const searchColumn = async(req, res, next) => {
  ...
  const result = await pool.query(`SELECT * FROM columns WHERE id::text ILIKE $1 OR name ILIKE $1 OR board_id::text ILIKE $1;`,[`%${search}%`]);
...
export { createColumn, getAllColumns, UpdateColumns, deleteColumn, searchColumn };
