...
export const createBoard = async (req, res, next) => {
  try {
    ...
    await pool.query(`INSERT INTO boards(title, user_id) VALUES ($1, $2)`, [title, user_id]);
...
export const getAll = async (req, res, next) => {
  ...
  try {
      ...
      const result = await pool.query(`SELECT * FROM boards LIMIT $1 OFFSET $2;`, [limit, offset]);
...
export const searchBoard = async(req,res,next) => {
  try{
    ...
    const result = await pool.query(`SELECT * FROM boards WHERE title ILIKE $1 OR user_id ::text ILIKE $1;`,[`%${search}%`]);

    if(result.rows.length === 0){
      return res.status(404).send({message:"Xech qanday ma'lumot topilmadi."});
