...
const createTasks = async (req, res, next) => {
  try {
    ...
    const boardCheck = await pool.query(`SELECT * FROM boards WHERE id=$1;`, [board_id]);
    if (boardCheck.rows.length === 0) {
      return res.status(400).json({ message: "Bunday board_id mavjud emas." });
    } 
    const columnCheck = await pool.query(`SELECT * FROM columns WHERE id=$1;`, [column_id]);
    if (columnCheck.rows.length === 0) {
      return res.status(400).json({ message: "Bunday column_id mavjud emas." });
    }
    
    const taskCheck = await pool.query(
      `SELECT * FROM tasks WHERE title=$1 AND description=$2 AND user_id=$3 AND board_id=$4 AND column_id=$5;`,
...
const getAllTasks = async (req, res, next) => {
  try {
    ...
    const totalResult = await pool.query(`SELECT COUNT(*) FROM tasks WHERE id::text ILIKE $1
       OR title ILIKE $1 OR description ILIKE $1 OR user_id::text ILIKE $1 OR board_id::text ILIKE $1
        OR column_id::text ILIKE $1;`,
      [`%${search}%`]
    );
    const total = parseInt(totalResult.rows[0].count);

    const result = await pool.query(`SELECT * FROM tasks WHERE title ILIKE $1
       OR description ILIKE $1
       OR user_id::text ILIKE $1 OR board_id::text ILIKE $1 
       OR column_id::text ILIKE $1 LIMIT $2 OFFSET $3;`, [
      `%${search}%`,
      limit,
      offset,
    ]);
